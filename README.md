...
Guided-scoping implementation coverage can be verified without overstating
professional acceptance:

```bash
fmr run-guided-scoping-acceptance \
  path/to/guided-scoping-corpus.json --output scoping-acceptance.json
```

The bundled corpus is synthetic. Production acceptance additionally requires a
representative anonymized corpus and a matching external practitioner review for
every hash-pinned case; FMR records the evidence references but does not
authenticate reviewer credentials.
